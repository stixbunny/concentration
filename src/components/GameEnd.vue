...
<template>
  <ModalDialog :condition="game.status">
    <form @submit.prevent="emit('newGame')" class="game-end p-4 flex flex-col gap-4">
      ...
      <button
        type="submit"
        class="game-end_submit rounded shadow bg-cyan-500 hover:bg-cyan-700 p-2 font-bold text-white focus:outline-none focus:shadow-outline"
      >
        Jugar de nuevo
      </button>
    </form>
